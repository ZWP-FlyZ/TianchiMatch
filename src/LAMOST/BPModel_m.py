...
#         train_loss = train_loss + tf.contrib.layers.l2_regularizer(.01)(W3);
    ...
    def train(self,datasource,learn_rate,steps,batch_size,load_value=False):
        ...
        lr = tf.train.exponential_decay(learn_rate, self.global_step, 200, 0.96, staircase=True);
        train_step = tf.train.AdamOptimizer(lr).minimize(self.loss,global_step=self.global_step);
        saver = tf.train.Saver(tf.global_variables());
        with tf.Session() as sess:
            if load_value:
                saver.restore(sess, 'value_cache/bpmode2.ckpt');
            else:
            ...
            for i in range(steps):
                ...
                if i%20 ==0:
                    ...
                    # print(py,y);
                    now = time.time();
            saver.save(sess,'value_cache/bpmode2.ckpt');
        print('finish!! ');
